...
{
    /**
     * Saves a Gateway schema.
     *
     * @param GatewaySchema $schema the schema to save
     * @param bool $runValidation Whether the schema should be validated
     * @return bool Whether the schema was saved successfully
     * @throws Exception
     */
    public function saveSchema(GatewaySchema $schema, bool $runValidation = true): bool
    {
        ...
        Craft::$app->getProjectConfig()->set($configPath, $configData, "Save Gateway schema “{$schema->name}”");
    ...
    /**
     * Deletes a Gateway schema by its ID.
     *
    ...
    /**
     * Deletes a Gateway schema.
     *
     * @param GatewaySchema $schema
     * @return bool
     */
    public function deleteSchema(GatewaySchema $schema): bool
    {
        Craft::$app->getProjectConfig()->remove(Constants::PATH_SCHEMAS . '.' . $schema->uid, "Delete the “{$schema->name}” Gateway schema");
        return true;
    }
     ...
     */
    public function getSchemaById(int $id): ?GatewaySchema
    {
        $result = $this->_createSchemaQuery()
            ->where(['id' => $id])
            ->one();

        return $result ? new GatewaySchema($result) : null;
    }
     ...
     * @return GatewaySchema|null
     */
    public function getSchemaByUid(string $uid): ?GatewaySchema
    {
        $result = $this->_createSchemaQuery()
            ->where(['uid' => $uid])
            ->one();
     ...
     * @return GatewaySchema|null
     */
    public function getSchemaByName(string $name): ?GatewaySchema
    {
        $result = $this->_createSchemaQuery()
            ->where(['name' => $name])
            ->one();
