...
namespace samuelreichor\gateway\services;
...
{
    /**
     * Saves a Gateway token.
     *
     * @param GatewayToken $token the schema to save
     * @param bool $runValidation Whether the schema should be validated
     * @return bool Whether the schema was saved successfully
     * @throws Exception
     * @throws Exception
     */
    public function saveToken(GatewayToken $token, bool $runValidation = true): bool
    {
        $isNewToken = !$token->id;
    ...
    /**
     * Deletes a Gateway token by its ID.
     *
    ...
    /**
     * Returns a Gateway token by its ID.
     *
     * @param int $id
     * @return GatewayToken|null
     */
    public function getTokenById(int $id): ?GatewayToken
    {
        $result = $this->_createTokenQuery()
            ->where(['id' => $id])
            ->one();

        return $result ? new GatewayToken($result) : null;
    }

    /**
     * Returns a Gateway token by its name.
     *
     * @param string $tokenName
     * @return GatewayToken|null
     */
    public function getTokenByName(string $tokenName): ?GatewayToken
    {
        $result = $this->_createTokenQuery()
    ...
    /**
     * Returns a Gateway token by its UID.
     *
     * @param string $uid
     * @return GatewayToken
     * @throws InvalidArgumentException if $uid is invalid
     */
    public function getTokenByUid(string $uid): GatewayToken
    {
        $result = $this->_createTokenQuery()
            ->where(['uid' => $uid])
            ->one();
        ...
        return new GatewayToken($result);
    }

    /**
     * Returns a Gateway token by its access token.
     *
     * @param string $token
     * @return GatewayToken
     * @throws UnauthorizedHttpException
     */
    public function getTokenByAccessToken(string $token): GatewayToken
    {
        $result = $this->_createTokenQuery()
    ...
    public function getSchemaUsageInTokens(int $schemaId): array
    {
        ...
        $usage = [];
        foreach ($rows as $row) {
            $token = new GatewayToken($row);
            $usage[] = [
                'name' => $token->name,
                'url' => UrlHelper::url('gateway/tokens/' . $token->id),
            ];
        }
